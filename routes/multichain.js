...
const router = express.Router();
  ...
  router.get('/getcurrencystate/:chainname/:blockstart/:blockend?/:blockintervals?/:converttocurrency?', async (req, res) => {
    try {
        const response = await fetch(config.VERUS_NODE_ADDRESS, {
            method: 'POST',
            headers: {
             ...
             body: '{"jsonrpc": "1.0", "id":"'+config.ID+'", "method": "getcurrencystate", "params": ["'+req.params.chainname+'", "'+req.params.blockstart+', '+req.params.blockend+', '+req.params.blockintervals+'","'+req.params.converttocurrency+'"] }'
          });
         const results = await response.json();
         res.json(results);
    } catch (err) {
